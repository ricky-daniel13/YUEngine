...
using UnityEngine;
public class SonicAnimator : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        anim.SetBool(idIsRoll, player.getRoll);
        anim.SetBool(idIsJump, player.getJump);
        ...
        //desiredSpeed = Quaternion.LookRotation(player.transform.rotation * player.input.desiredDir, player.transform.up) * Quaternion.Inverse(Quaternion.LookRotation(fwr, player.transform.up)) * Vector3.forward;

        //currDesired = Vector3.MoveTowards(currDesired, player.input.heldPower > 0 ? desiredSpeed : Vector3.zero, inputChangeSpeed * Time.deltaTime);
