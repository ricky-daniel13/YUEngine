...
{
    public float radSpeed, azimSpeed, elevSpeed, defRad, defAzim, defElev, lookSpeed,fovSpeed,fovNormal,fovFast,maxAngle,angleSpeed, playerOffset, minRunSpeed, maxRunSpeed, upAdjustSpeed,elevPhysSpeed;
    float rad, azim, elev, radVel, azimVel, elevVel,fovVel,currFov,currentUpDot;
    ...
    private void LateUpdate() {
        ...
        if(player.actionState.StateName == "Jump"||followOnSlopes){
            float upSpeedDot = Vector3.Dot(Vector3.up, playerVelocity);
            if(upSpeedDot > 0||followOnSlopes&&player.player.GetIsGround){
        ...
        localMat.SetTRS(player.transform.position + (Vector3.up*currentUpDot*playerOffset), Quaternion.FromToRotation(Vector3.up, currentUp), Vector3.one);
        ...
        float targetElev = Mathf.Asin(Mathf.Max(Mathf.Min(autoAdjustElev ? defElev : localCamPos.y, rad-Mathf.Epsilon),-(rad-Mathf.Epsilon))/rad);
        
        if(physAdjustElev)
            elev = Mathf.Deg2Rad*Mathf.MoveTowardsAngle(elev*Mathf.Rad2Deg, targetElev*Mathf.Rad2Deg, elevPhysSpeed*Time.deltaTime);
        else
